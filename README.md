...
A tiny multi-agent coordinator for [herdr](https://herdr.dev). One Claude Code agent (the **head**) receives your tasks, delegates each one to a **worker** agent — Claude, Codex, Gemini, or any other herdr-supported kind — in its own herdr tab, tracks their lifecycle, and collects their reports.
